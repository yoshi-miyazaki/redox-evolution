...
''' element information '''
Nmol = 7
nO, nMg, nAl, nSi, nV, nFe, nNi = 0, 1, 2, 3, 4, 5, 6
list_major = [nMg, nAl, nSi, nFe, nNi]
list_minor = [nV]

# O, Mg, Si, V, Fe, Ni
molar_mass = np.array([16, 24.3, 26.98, 28., 50.94, 55.8, 58.7])*1e-3
...
def set_initial_mantle_composition(x_oxide):
    ...
    m_element   = np.array([m_oxide[0] + m_oxide[1]*3 + m_oxide[2]*2 + m_oxide[3]*3 + m_oxide[4] + m_oxide[5], # O
                            m_oxide[0],    # Mg
                            m_oxide[1]*2,  # Al
                            m_oxide[2],    # Si
                            m_oxide[3]*2,  # V
